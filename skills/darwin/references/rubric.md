...
- 改进后总分必须 **严格高于** 改进前才保留（保留1位小数，不靠四舍五入）

## 关于「实测表现」维度

评分方式：

1. 为每个skill设计2-3个**典型用户prompt**（最常见使用场景）
...
如果无法跑子agent，退化为「干跑验证」：模拟典型prompt的执行思路，判断流程是否合理。在results.tsv中标注 `dry_run`。
